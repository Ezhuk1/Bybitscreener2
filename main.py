...
def build_bybit_deeplink(symbol: str) -> str:
    base = "https://bybit.onelink.me/EhY6"
    dp = f"bybitapp://open/home?tab=2&symbol={symbol}"
    return (
        f"{base}?af_xp=custom&pid=uj"
        f"&af_dp={requests.utils.quote(dp)}"
        f"&is_retargeting=true&c=h5_trading_popup&af_force_deeplink=true"
    )

def send_tg_signal(symbol: str, change: float, close_price: float, turnover: float):
    url_bybit = build_bybit_deeplink(symbol)
    ...
    payload = {
        ...
        "reply_markup": {
            "inline_keyboard": [[
                {"text": "📈 Открыть в приложении", "url": url_bybit},
